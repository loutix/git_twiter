...
# envoyer 3 tweet à 3 adresses random. 

#3.times do |variable|

#		client.update("#{journalistes[rand(journalistes.length)]} #bonjour_monde") 
#end


# 2.2 liker (#bonjour monde) 25 fois.


# client.search('#bonjour_monde').take(3).each do |tweet|
# client.fav tweet
# end

#2.3 tweeter les 20 dernières personnes qui ont twetté #bonjour mode
#client.search('#tf1', result_type:  "recent" ).take(3).each do |tweet|
...
# utiliser la fonction live de twitter, streaming = liker + follow 
...
client_stream.filter(track:"#covid19") do |object|
